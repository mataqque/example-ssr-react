...
export class LeadService {
	...
	async save() {
		let payload = {
			...
			observation: [this.#lead.dorms && `Dormitorios: ${this.#lead.dorms}`, this.#lead.motive && `Motivo: ${this.#lead.motive}`, this.#lead.salary && `Monto a invertir : ${this.#lead.salary}`]
				.filter(Boolean)
				.join(', '),
			input_channel_id: 4,
			source_id: 95,
	...
	sendMailToInterested() {
		return this.#mail.from(`Momen <no-reply@momen.pe>`).to(this.#lead.email).subject('Te damos la bienvenida').html(WelcomeMail(this.#lead)).send();
	}
}
